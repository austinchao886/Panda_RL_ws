...
REG_TASK = 'panda_env_cfg'   ## This place will be the class name of the training
...
parser.add_argument("--config", type=str, default=f'panda_robot/configs/{REG_TASK}.py', help="")
...
def save_configurations(log_dir, REG_TASK):
    ...
    panda_env_cfg_file = f"{root}/panda_robot/configs/{REG_TASK}.py"
    panda_robot_cfg_file = f"{root}/panda_robot/configs/panda_robot_cfg.py"
    panda_scene_cfg_file = f"{root}/panda_robot/configs/panda_scene_cfg.py"
    ppo_cfg_file = f"{root}/panda_robot/agents/rsl_rl_ppo_cfg.py"

    # copy files
    new_path = lambda x, log_dir: f"{log_dir}/{Path(x).name}"
    configs_dir = f"{log_dir}/configs"
    os.makedirs(configs_dir, exist_ok=True)
    shutil.copyfile(panda_env_cfg_file, new_path(panda_env_cfg_file, configs_dir))
    shutil.copyfile(panda_robot_cfg_file, new_path(panda_robot_cfg_file, configs_dir))
    shutil.copyfile(panda_scene_cfg_file, new_path(panda_scene_cfg_file, configs_dir))
...
def register_env_task(env_task):
    ...
    gym.register(
        ...
        kwargs={
            "env_cfg_entry_point": f"panda_robot.configs.{env_task}:PandaEnvCfg",
            "rsl_rl_cfg_entry_point": "panda_robot.agents.rsl_rl_ppo_cfg:VanilaPPORunnerCfg",
        },
    )
...
def main():
    ...
    env_cfg.scene.env_spacing = args_cli.env_spacing if args_cli.env_spacing is not None else env_cfg.scene.num_envs
    ...
    log_dir = datetime.now(tw_tz).strftime("%Y-%m-%d_%H_%M_%S")
