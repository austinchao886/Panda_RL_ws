...
parser.add_argument("--log_dir", type=str, default="/workspace_v2/scripts/walk/logs/panda_env_cfg/rsl_rl/ppo_runner/2026-04-16_17_46_28", help="")
...
def register_env_task(env_task, log_dir):
    ...
    gym.register(
        ...
        kwargs={
            "env_cfg_entry_point": f"{entry_cfg_point}.configs.{env_task}:PandaEnvCfg",
...
def get_latest_pt(path_input):
    ...
    # return Path(path_input).__str__() + f"/model_500.pt"
...
def main():
    ...
    # action_bias = torch.tensor([0.02901, -0.00215, -0.99794, 1.0, 0.0, 0.0, 0.0], 
    #                     device= env.unwrapped.device ,dtype=torch.float32)
    # action_bias_tensor = action_bias.unsqueeze(0).repeat(env.num_envs, 4)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            actions_with_bias = policy(obs)
            actions = actions_with_bias # + action_bias_tensor


            # print(actions.shape)
            # print(f"actions:{actions[:,0:3]}")
